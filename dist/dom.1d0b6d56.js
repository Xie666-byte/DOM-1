...
window.dom = (_window$dom = {
  ...
  create: function create(string) {
    //相当于window.dom.create,string是要传入的节点
    var container = document.createElement("template");
    container.innerHTML = string.trim(); //将要传入的节点传给目标节点，上面目标节点是template，这个标签可以容纳任意的元素;trim()可以去掉目标节点中的空格

    return container.content.firstChild; //创建节点时，在id为test的div外面加一个div才有有效，否则就会报错，具体原因未知
  },
  ...
  empty: function empty(node) {
    var childNodes = node.childNodes; //等价于const childNodes = node.childNodes;

    var array = [];
    var x = node.firstChild; //第一个节点
...
}, _defineProperty(_window$dom, "remove", function remove(node, className) {
  node.classList.remove(className);
}), _defineProperty(_window$dom, "has", function has(node, className) {
